...
async function chatAboutProject(project, clips, history, question) {
  ...
  const system =
    ...
    '}\n' +
    'Each quote MUST be copied word-for-word from a transcript (so it can be located in the text) and kept short — just the relevant sentence or two. ' +
    'Include one source per distinct fact you used. If the answer is not in the recordings, say so in "answer" and return an empty "sources" array. ' +
