...
if (!recordingCols.includes('dates')) {
  db.exec('ALTER TABLE recordings ADD COLUMN dates TEXT');
}
if (!recordingCols.includes('segments')) {
  db.exec('ALTER TABLE recordings ADD COLUMN segments TEXT');
}
...
if (!projectCols.includes('dates')) {
  db.exec('ALTER TABLE projects ADD COLUMN dates TEXT');
}
if (!projectCols.includes('share_token')) {
  db.exec('ALTER TABLE projects ADD COLUMN share_token TEXT');
}

const messageCols = db.prepare('PRAGMA table_info(project_messages)').all().map((c) => c.name);
if (!messageCols.includes('sources')) {
  db.exec('ALTER TABLE project_messages ADD COLUMN sources TEXT');
}
...
const setShareTokenStmt = db.prepare('UPDATE projects SET share_token = ? WHERE id = ?');

function setProjectShareToken(id, token) {
  setShareTokenStmt.run(token, id);
  return getProject(id);
}

const getByShareTokenStmt = db.prepare('SELECT * FROM projects WHERE share_token = ?');

function getProjectByShareToken(token) {
  if (!token) return null;
  return hydrateProject(getByShareTokenStmt.get(token));
}
...
function setProjectAnalysis(id, { summary, insights, next_steps, dates }) {
...
function setRecordingResult(id, { transcript, summary, insights, next_steps, dates, segments, duration }) {
  setResultStmt.run({
    id,
    transcript,
    summary,
...
const addMessageStmt = db.prepare(`
  INSERT INTO project_messages (project_id, role, content, sources, created_at)
  VALUES (@project_id, @role, @content, @sources, @created_at)
`);

function addProjectMessage(projectId, role, content, sources = null) {
...
function listProjectMessages(projectId) {
  return listMessagesStmt.all(projectId).map((m) => ({
    ...m,
